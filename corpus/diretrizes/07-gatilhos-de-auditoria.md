...
> A coluna `Efeito` diz o mesmo padrão sem jargão, e é a forma que casa com material em prosa. **A tradução não pode ser mais larga que o gatilho.** Ela tem de carregar a premissa dele, senão dispara onde o gatilho não dispararia. Medido: o `Efeito` do G69 saiu como "ninguém fixou em que país o dado é processado" e passou a acusar um projeto sem dado de paciente nenhum, onde o país não importa. A redação corrigida nomeia o dado — "o dado do paciente é processado no país que o fornecedor escolher". Quem escrever uma tradução nova confere isto antes: o gatilho vale só quando X; a tradução também tem de dizer X.
...
| G09 | índice vetorial ou retriever sem filtro por paciente na consulta | `risco` | SEC:llm.vetores-clinicos | o isolamento está no índice ou só no prompt | filtrar por paciente no próprio índice, não no prompt | — | a busca traz trecho de prontuário de outro paciente |
| G10 | conteúdo externo — PDF, e-mail, laudo — no contexto de agente com ferramenta de rede ou de escrita | `risco` | SEC:llm.owasp-top10 · SEC:llm.agentic | o que o agente faz se o documento trouxer instrução | isolar conteúdo de terceiro do canal de instrução do agente | — | documento vindo de fora traz instrução escondida, e o sistema que age sozinho obedece |
| G11 | saída do modelo renderizada como HTML ou interpolada em SQL, shell ou `eval` | `risco` | SEC:llm.owasp-top10 | há sanitização entre a saída e o destino | sanitizar a saída do modelo antes de qualquer destino executável ou renderizado | — | o texto que o modelo devolve é executado, ou exibido como página, sem conferência |
| G12 | interface que carrega recurso externo citado na saída — imagem, iframe, fetch | `risco` | SEC:llm.owasp-top10 | o dado sai na URL do recurso | bloquear o carregamento de recurso externo citado pela saída | — | a tela busca uma imagem ou endereço citado pelo modelo, e o dado do paciente vai junto no caminho |
| G13 | agente com credencial de escrita em produção, token admin ou HTTP arbitrário | `risco` | SEC:llm.agentic | o que executa sem aprovação humana | retirar credencial de escrita do agente e exigir aprovação humana | — | o sistema altera dado do paciente sozinho, sem ninguém aprovar |
...
| G15 | notebook ou pipeline de pesquisa que chama API externa sobre coorte identificada | `bloqueante` | LGPD:art13 | o estudo se ampara no art. 13 | amparar o estudo no art. 13, ou desidentificar antes da chamada | — | pesquisa manda um grupo de pacientes identificados para serviço de fora |
...
| G19 | desidentificação que remove apenas identificadores diretos | `risco` | LGPD:art12 · SEC:anonimizacao.quase-identificadores | os quase-identificadores foram tratados | tratar os quase-identificadores, não apenas os identificadores diretos | — | a desidentificação tirou o nome e deixou o que ainda identifica: idade, data, cidade |
...
| G21 | `evolucao`, `historia`, `observacao`, `laudo` ou outra coluna de texto livre saindo da base sem tratamento | `risco` | SEC:anonimizacao.quase-identificadores · CEM:art73 | o texto livre recebe o mesmo tratamento das colunas estruturadas, ou não há tratamento nenhum | passar o texto livre pela mesma rotina aplicada às colunas estruturadas | — | o texto que o médico escreveu sobre o paciente sai inteiro, e é ali que está o nome |
...
| G25 | base dita anonimizada, ou destinada a compartilhamento, sem método e medição | `risco` | LGPD:art12 · SEC:anonimizacao.evidencia | quem assinou, e há classe com k=1 | medir o risco residual e registrar método, responsável e data | — | alguém diz que a base está anonimizada e não há como conferir |
...
| G29 | credencial de prontuário compartilhada entre pessoas ou embutida em automação | `bloqueante` | CEM:art87 · CFM-1821-2007:art3 · LGPD:art46 | cada acesso é atribuível a uma pessoa | credencial nominal por pessoa, com registro de cada acesso ao prontuário | — | a mesma senha do prontuário serve para várias pessoas, ou está gravada dentro de um programa |
...
| G31 | segredo ou regra confidencial dentro do system prompt | `risco` | SEC:llm.owasp-top10 | o system prompt é recuperável pela saída | retirar o segredo do system prompt e tratá-lo como conteúdo recuperável | — | a instrução fixa do sistema guarda senha ou regra que não deveria sair, e o modelo pode repeti-la |
...
| G33 | ausência de trilha das chamadas — categoria de dado que saiu, momento, destino, referência do registro clínico | `risco` | LGPD:art37 · ANPD-15-2024:art6 | como se contam os titulares afetados | registrar categoria de dado, momento, destino e referência ao registro clínico | — | dado de paciente sai para o fornecedor e não fica registro de qual, quando e de qual atendimento |
| G34 | trilha de IA só no provedor, ou com retenção inferior à do registro clínico | `risco` | CEM:art6 · CFM-1821-2007:art7 | dá para exportar sob requisição do CRM | manter trilha própria, com retenção igual à do registro clínico | — | o único registro do uso de IA está com o fornecedor, e some antes do prontuário |
...
| G38 | chave literal no código, ou no mesmo repositório, banco ou volume do dado cifrado | `bloqueante` | LGPD:art46 · SEC:repouso.chaves | quem acessa a chave acessa o dado | separar chave e dado cifrado, com custódia e rotação próprias | — | a chave que abre o dado cifrado está guardada junto do próprio dado |
| G39 | base com dado de paciente sem criptografia em repouso | `risco` | LGPD:art46 · CFM-2454-2026:anexoI.XV-XVI | o dado ficaria ininteligível a terceiro | cifrar em repouso, incluindo backup e réplica | CFM-2454-2026 | o banco com dado de paciente está gravado sem cifra |
| G40 | cifragem declarada só por disco ou TDE com campos em claro, ou backup e réplica sem cifragem | `risco` | SEC:repouso.camadas | a declaração corresponde ao implementado | cifrar em camada de aplicação ou de campo, além do disco | — | a cifragem declarada não alcança o campo, o backup nem a réplica |
...
| G44 | registro clínico com prazo de expiração definido pelo provedor | `bloqueante` | CFM-1821-2007:art7 · CEM:art90 | a instituição mantém cópia exportável | manter cópia exportável sob controle da instituição, independente do provedor | — | o fornecedor decide por quanto tempo o registro clínico existe |
...
| G47 | conduta, dose ou diagnóstico entregue ao paciente por canal automático — chatbot, e-mail, portal — sem médico entre a saída do modelo e o paciente | `bloqueante` | CEM:art37 · CFM-2454-2026:art5 | há mediação humana antes do envio | mediação humana antes de qualquer envio de conduta ao paciente | CFM-2454-2026 | o paciente recebe conduta ou diagnóstico direto da máquina |
...
| G78 | resultado apoiado em IA entregue ao paciente como conclusão do serviço, sem médico identificado que responda por ele | `risco` | CFM-2454-2026:art10 · CEM:art87 | quem é o médico responsável pelo resultado, e como o paciente o alcança | identificar o médico responsável no resultado entregue ao paciente | CFM-2454-2026 | o paciente recebe um resultado feito com IA sem saber qual médico responde por ele |
...
| G80 | ausência de trilha que permita reconstruir uma chamada passada ao modelo, fora do registro clínico | `risco` | CFM-2454-2026:art9 | dá para reconstruir o que o sistema respondeu num atendimento passado | trilha própria da chamada, distinta do registro clínico do art. 4º, V | CFM-2454-2026 | não dá para reconstruir o que o sistema respondeu num atendimento passado |
...
| G52 | saída de IA que influenciou a decisão registrada, persistida fora do prontuário | `bloqueante` | CEM:art87 · CFM-1821-2007:art7 | o conteúdo entra na exportação do prontuário | trazer para o prontuário a saída de IA que influenciou a decisão registrada | — | a conversa que produziu a decisão fica num sistema que não é o prontuário |
...
| G54 | triagem, priorização de fila ou negativa gerada por modelo, sem canal de revisão | `risco` | LGPD:art20 | os critérios da decisão estão documentados | canal de revisão humana, com os critérios da decisão documentados | — | o modelo decide fila, prioridade ou negativa, e não há quem revise |
...
| G62 | mecanismo de transferência declarado é o consentimento, e o termo não informa o caráter internacional | `bloqueante` | LGPD:art33 | qual mecanismo do art. 33 o fluxo declarou | informar o caráter internacional da transferência, ou adotar outro mecanismo do art. 33 | — | o paciente consentiu, e o termo não diz que o dado sai do país |
...
| # | Gatilho | Severidade | Base | O que checar | Mitigação | Norma | Efeito |
|---|---|---|---|---|---|---|---|
| G64 | teste, homologação ou avaliação com dado real — dump de produção, seed, planilha | `bloqueante` | LGPD:art46 · SEC:repouso.camadas | quem acessa, e há gerador de dado sintético | gerador de dado sintético para teste, homologação e avaliação | — | dado de paciente real está em ambiente de teste |
| G65 | prompt de exemplo, issue, wiki ou captura de tela com caso real | `bloqueante` | CEM:art75 · CP:art154 | o conteúdo já está público ou versionado | remover o conteúdo e tratar a exposição como incidente | — | caso real de paciente aparece em exemplo, chamado interno ou captura de tela |
| G66 | fine-tuning ou few-shot a partir de caso real identificável | `bloqueante` | LGPD:art11 · CEM:art89 | qual base e qual autorização amparam | base legal e autorização escrita, ou treinar com dado sintético | — | caso real de paciente foi usado para treinar o modelo ou como exemplo fixo |
...
| G69 | variável de região ausente, com padrão do fornecedor, ou com roteamento automático | `risco` | LGPD:art33 · ANPD-19-2024:art3.III | a região é lida da configuração em execução | fixar a região na configuração, lida em execução | — | o dado do paciente é processado no país que o fornecedor escolher, e ninguém fixou qual |
...
| G72 | runbook que dispensa comunicação ao titular por os dados estarem criptografados | `bloqueante` | ANPD-15-2024:art9 · LGPD:art48 | a dispensa tem base na Res. 15/2024 | rever a dispensa de comunicação contra a Res. ANPD 15/2024 | — | o plano de incidente dispensa avisar o paciente porque o dado estava cifrado |
...
| G76 | ausência de evidência de orientação da equipe quanto ao sigilo — política, treinamento ou termo | `risco` | CEM:art78 | quem responde pelo uso por não médicos | política interna e treinamento da equipe quanto ao sigilo, com evidência | — | não há prova de que a equipe foi orientada quanto ao sigilo |
| G77 | descrição clínica ou CID em cobrança, agendamento ou faturamento, como dado não sensível | `risco` | CEM:art79 · LGPD:art11§1-§3 | o fluxo financeiro segrega o campo clínico | segregar o campo clínico do fluxo financeiro | — | o diagnóstico do paciente circula no setor financeiro como se fosse dado comum |
